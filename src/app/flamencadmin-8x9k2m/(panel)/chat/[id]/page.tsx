...
interface Conversation {
  ...
  buyer_id: string;
  seller_id: string;
  product: { id: string; title: string; images: string[]; price: number; status: string } | null;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex items-center gap-5 text-sm">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[10px] text-flamencalia-black/40">Comprador</p>
                <p className="font-medium text-flamencalia-black text-xs">
                  {buyer?.display_name || "—"}
                </p>
              </div>
            </div>
            <svg className="w-4 h-4 text-flamencalia-albero-pale" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
            </svg>
            <div className="flex items-center gap-2">
              <Avatar url={seller?.avatar_url} name={seller?.display_name} />
              <div>
                <p className="text-[10px] text-flamencalia-black/40">Vendedor</p>
                <p className="font-medium text-flamencalia-black text-xs">
                  {seller?.display_name || "—"}
                </p>
              </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1 pb-2">
        {messages.length === 0 ? (
          ...
          messages.map((msg) => {
            ...
            const alignment = isAdmin ? "justify-center" : isBuyer ? "justify-start" : "justify-end";
            ...
            const badgeLabel = isAdmin ? "Admin" : isBuyer ? "Comprador" : "Vendedor";
            ...
            return (
              <div key={msg.id} className={`flex gap-2.5 ${alignment}`}>
                ...
                <div className="max-w-sm lg:max-w-md">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-medium text-flamencalia-black/40">
                      {isAdmin ? "Administrador" : sender?.display_name || "Usuario"}
                    </span>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${badgeClass}`}>
                      {badgeLabel}
                    </span>
                  </div>
                  <div className={`px-4 py-2.5 text-sm leading-relaxed ${bubbleClass}`}>
                ...
                {isSeller && (
                  <Avatar url={sender?.avatar_url} name={sender?.display_name} size="sm" />
                )}
              </div>
            );
          })
      ...
      <div className="shrink-0 pt-3 border-t border-flamencalia-albero-pale/15 mt-2">
        <div className="flex items-center gap-2 mb-1.5">
          <div className="w-5 h-5 rounded-full bg-flamencalia-albero/20 flex items-center justify-center">
            <svg className="w-3 h-3 text-flamencalia-albero" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
...
function Avatar({
  ...
  if (isAdmin) {
    return (
      <div className={`${dim} rounded-full bg-flamencalia-albero/20 flex items-center justify-center shrink-0`}>
        <svg className="w-3.5 h-3.5 text-flamencalia-albero" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
        </svg>
      </div>
    );
